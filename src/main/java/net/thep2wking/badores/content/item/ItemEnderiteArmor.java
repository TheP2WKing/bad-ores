...
public class ItemEnderiteArmor extends ModItemArmorBase {
	...
	public void onEntityAttacked(LivingAttackEvent event) {
		if (event.getEntityLiving() instanceof EntityPlayer) {
			...
			if (head != null && head.getItem() instanceof ItemEnderiteArmor) {
				ModItemEvents.enderiteArmorTeleport(world, player, player.getPosition());
			}
			if (chest != null && chest.getItem() instanceof ItemEnderiteArmor) {
				ModItemEvents.enderiteArmorTeleport(world, player, player.getPosition());
			}
			if (legs != null && legs.getItem() instanceof ItemEnderiteArmor) {
				ModItemEvents.enderiteArmorTeleport(world, player, player.getPosition());
			}
			if (feet != null && feet.getItem() instanceof ItemEnderiteArmor) {
				ModItemEvents.enderiteArmorTeleport(world, player, player.getPosition());
			}
		}
	}
